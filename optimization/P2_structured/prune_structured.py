...
def apply_structured_pruning(model, amount=0.3):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and module.weight.shape[0] > 1:
            ...
            pruned_count += 1
    print(f"  Pruned {pruned_count} Conv2d layers (structured, dim=0)")
    return model
...
def main():
    ...
    print(f"Applying structured pruning (amount={PRUNE_AMOUNT})...")
    model = apply_structured_pruning(model, PRUNE_AMOUNT)
    ...
    torch.save(model.state_dict(), "model_p2.pt")
    p_size = os.path.getsize("model_p2.pt") / 1024 / 1024
    b_size = os.path.getsize(BASELINE_PT) / 1024 / 1024

    print(f"\n── P2 Structured Pruning Results ──")
    ...
    results = {
        "technique": "P2 - Structured Pruning (30%)",
    ...
    with open("results_p2.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nResults saved to results_p2.json")
