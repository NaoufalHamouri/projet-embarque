...
def main():
    ...
    print(f"Applying unstructured pruning (amount={PRUNE_AMOUNT})...")
    model = apply_unstructured_pruning(model, PRUNE_AMOUNT)
    ...
    p_size = os.path.getsize("model_p1.pt") / 1024 / 1024
    b_size = os.path.getsize(BASELINE_PT) / 1024 / 1024

    print(f"\n── P1 Unstructured Pruning Results ──")
    ...
    results = {
        "technique": "P1 - Unstructured Pruning (30%)",
    ...
    with open("results_p1.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nResults saved to results_p1.json")
